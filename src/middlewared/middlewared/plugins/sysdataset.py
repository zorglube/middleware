    ...
    async def setup(self, exclude_pool):
        ...
        if not config['pool_set']:
            if pool := await self._query_pool_for_system_dataset(exclude_pool):
                self.logger.debug(
                    'System dataset pool was not set, moving it to first available pool %r', pool['name']
                )
        ...
        if mounted_pool and mounted_pool != config['pool']:
            self.logger.debug('Moving system dataset from %r to %r', mounted_pool, config['pool'])
